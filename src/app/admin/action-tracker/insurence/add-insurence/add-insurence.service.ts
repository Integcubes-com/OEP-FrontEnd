...
export class AddInsurenceService {
  ...
  getRecommendationsList(userId: number,regionList:string, siteList:string, sourceList:string, nomacStatus:string, insuranceStatus:string, priorityList:string): Observable<InsurenceRecommendationApi> {
    let data = { userId, regionList, siteList, sourceList, nomacStatus, insuranceStatus, priorityList}
    return this.http.post<InsurenceRecommendationApi>(this.getnsurenceRecommendationsURL, data)
  }
